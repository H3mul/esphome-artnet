...
class ArtNetReceiver : public Component {
 public:
  ...
  void set_artnet_universe(uint16_t universe) { this->artnet_universe_ = universe; }
  void add_channel(ArtNetChannel *channel) { this->channels_.push_back(channel); }

 protected:
  void on_dmx_frame(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t *data);
  static void on_dmx_frame_callback(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t *data);
