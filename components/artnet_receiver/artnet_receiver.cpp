...
void ArtNetReceiver::setup() {
  ...
  ESP_LOGCONFIG(TAG, "ArtNet Receiver setup complete. Listening on universe %d", this->artnet_universe_);
}
...
void ArtNetReceiver::dump_config() {
  ...
  for (auto *channel : this->channels_) {
    ESP_LOGCONFIG(TAG, "    Channel %d: %s", 
                 channel->get_channel_number(), 
                 channel->get_name().c_str());
  }
}

void ArtNetReceiver::on_dmx_frame(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t *data) {
  // Check if this is the universe we're interested in
  if (universe != this->artnet_universe_) {
    return;
  }
  
  ESP_LOGV(TAG, "Received DMX frame for universe %d, length %d, sequence %d", universe, length, sequence);
...
void ArtNetReceiver::on_dmx_frame_callback(uint16_t universe, uint16_t length, uint8_t sequence, uint8_t *data) {
